...
def prepare_inputs_for_bert_xlnet(input_sentences, tokenizer, bos_eos=False, cls_token_at_end=False, pad_on_left=False, pad_token=0, sequence_a_segment_id=0, cls_token_segment_id=1, pad_token_segment_id=0, device=None, feed_transformer=False):
    """
    ...
    return tokens_tensor, segments_tensor, input_mask, gather_indexes, remove_cls_seq_indexes, aggregated_indexes, aggregated_counts

def transformer_forward_by_ignoring_suffix(transformer, input_ids, segment_ids, attention_mask, gather_index, remove_cls_seq_index, aggregated_index, aggregated_count, max_word_seq_length, device=None, alignment='first'):
    '''
    Ignore hidden states of all suffixes: [CLS] from ... to de ##n ##ver [SEP] => from ... to de
    '''
    
    assert alignment in {'first', 'avg'}
